...
import "../../interfaces/leverageX/libraries/IFeeTiersUtils.sol";
import "../../interfaces/leverageX/types/IFeeTiers.sol";
...
library FeeTiersUtils {
    ...
    function setGroupVolumeMultipliers(
        ...
        mapping(uint256 => uint256) storage groupVolumeMultipliers = _getStorage()
    ...
    function updateTraderPoints(address _trader, uint256 _volumeUsd, uint256 _groupIndex) internal {
        ...
        uint224 points = uint224(
            (_volumeUsd * s.groupVolumeMultipliers[_groupIndex]) / GROUP_VOLUME_MULTIPLIER_SCALE
        );

        mapping(uint32 => IFeeTiers.TraderDailyInfo) storage traderDailyInfo = s.traderDailyInfos[
            _trader
        ];
        ...
        if (currentDay > traderInfo.lastDayUpdated) {
            ...
            for (uint256 i = getFeeTiersCount(); i > 0; --i) {
                ...
                if (
                    curTrailingPoints >= uint224(feeTier.pointsThreshold) * POINTS_THRESHOLD_SCALE
                ) {
